...
import ch.thn.datatree.onoff.core.GenericOnOffSetTreeNode;
...
extends GenericOnOffSetTreeNode<V, OnOffSetTreeNode<V>> {
  ...
  public OnOffSetTreeNode<V> nodeFactory(V value) {
    return new OnOffSetTreeNode<V>(value);
  }

  @Override
  public OnOffSetTreeNode<V> nodeFactory(OnOffSetTreeNode<V> node) {
    return new OnOffSetTreeNode<V>(node.getComparator(), node.getNodeValue());
  }
