 ...
 * 
 */
package ch.thn.datatree.printer.generic;

import java.util.ArrayList;
import java.util.Collection;
import java.util.List;

import ch.thn.datatree.printer.TreePrinterUtil;
 ...
 *
 */
public abstract class PlainTextTreePrinter<N> 
	extends GenericTreePrinter<	N, 
								String, 
								StringBuilder, 
								List<String>> {
	...
	protected List<TreePrinterLine<String>> createPrinterLines(N node, List<String> prefix,
		...
		List<TreePrinterLine<String>> lines = new ArrayList<TreePrinterLine<String>>();
		...
		TreePrinterLine<String> line = new TreePrinterLine<String>(siblingIndex, siblingCount, 0, nodeValues.size(), childrenCount, depth);
		lines.add(line);
		...
		for (String s : nodeValues) {
			...
			if (nodeValueIndex != 0) {
				...
				TreePrinterLine<String> additionalLine = new TreePrinterLine<String>(siblingIndex, siblingCount, nodeValueIndex, nodeValues.size(), childrenCount, depth);
