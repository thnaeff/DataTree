...
public abstract class GenericTreePrinter<N, V, O, R> {
	...
	public O print(N node) {
		...
		List<TreePrinterLine<V>> output = createPrinterLines(node, null, 0, 1, size, 0);
	...
	protected List<List<TreePrinterLine<V>>> printChildren(N node, R runningValue, int depth) {
		ArrayList<List<TreePrinterLine<V>>> printed = new ArrayList<List<TreePrinterLine<V>>>();
		...
		for (N child : children) {
			Collection<N> childChildren = getChildren(child);
			int size = (childChildren == null ? 0 : childChildren.size());
